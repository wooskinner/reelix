...
function generateReport() {
  const bundleSizesPassed = checkBundleSizes();

  const report = {
    timestamp: new Date().toISOString(),
    budgets: BUDGETS,
    checks: {
      bundleSizes: bundleSizesPassed,
    },
    passed: bundleSizesPassed
  };
  ...
  console.log(
    `\n${report.passed ? '✅' : '❌'} Performance report written to performance-report.json`
  );
...
// Exit with a non-zero code on failure so this can be used as a CI gate
